...
def get_Edet(
    log_Etrue: np.ndarray, en_unc: float, Eth: float, Emax: float
) -> np.ndarray:
...
def get_mean_lnA_det(
    mean_lnA_true: np.ndarray, mean_lnA_unc: Union[np.ndarray, float] = 2
) -> np.ndarray:
...
def get_var_lnA_det(
    var_lnA_true: np.ndarray, var_lnA_unc: Union[np.ndarray, float] = 0.5
) -> np.ndarray:
...
def get_direction_acceptance(
    coord_earth: np.ndarray,
    kappa_det: float,
    detector_params: tuple,
    max_exposure: float,
    accept_all_dirs : bool = False,
) -> Tuple[int, SkyCoord, float]:
    """
    ...
    detector_params : tuple
        The parameters of the detector exposure function.
    max_exposure : float
        The maximum exposure of the detector.
    accept_all_dirs : bool
        If True, all directions are accepted (for all-sky detectors).
    ...
    reconstr_uv = sample_vMF(coord_earth, kappa_det, num_samples=1).T.squeeze()
    ...
    m_omega = m_dec(reconst_coord.icrs.dec.rad, detector_params)
    reconst_coord.transform_to("galactic")
    reconst_coord.representation_type = "unitspherical"
    if accept_all_dirs:
        return 1, reconst_coord, 1.0
    
    pdet = m_omega / max_exposure
    probs = np.array([pdet, 1.0 - pdet]) / np.sum(np.array([pdet, 1.0 - pdet]))
...
def simulate_zenith_angles(c_icrs, zen_thresh=None, period_start=None, location=None):
    ...
    for d in c_icrs:
        ...
        while za > zen_thresh:
            ...
            c_altaz = d.transform_to(AltAz(obstime=tdy, location=location))
