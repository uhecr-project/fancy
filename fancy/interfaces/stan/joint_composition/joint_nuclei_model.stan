...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    for (k in 1:Ns+1) {
      ...
      if ((k < Ns+1) ) {
        ...
        kappas[i] = get_kappa(R[i], pow(10.0, log10_Bigmf), D_kappa[k]);
        if (kappas[i] > 1e6) {
          kappas[i] = 1e6;
        }
  ...
  for (k in 1:Ns+1) {
    ...
    if ((k < Ns+1) ) {
      ...
      log10_wexp = interp2d(alpha_s, log10_Bigmf, to_array_1d(alpha_grid), to_array_1d(log10_Bigmf_grid),  log10_wexp_src_grid[k,:,:]);
    }
    else {
      log10_wexp = interpolate(alpha_grid, to_vector(log10_wexp_bg_grid), alpha_b);
    }

    log10_wexp_arr[k] = log10_wexp;
