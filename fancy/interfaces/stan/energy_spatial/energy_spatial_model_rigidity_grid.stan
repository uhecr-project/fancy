...
// --- per-event likelihood chunk for reduce_sum ---
  real event_likelihood_chunk(
      ...
      vector mean_lnA_det,         // mean lnA per energy bin per source
      vector var_lnA_det,          // variance of lnA per energy bin per source
      ...
      for (n in 1:len) {
          ...
          real mean_lnA = interpolate(lnA_logE_grid_vec, mean_lnA_det, logE_true[i]);
          real var_lnA  = fmax(interpolate(lnA_logE_grid_vec, var_lnA_det, logE_true[i]), 0.0);
...
data {
    ...
    vector[NEbins] mean_lnA_det;
    vector[NEbins] var_lnA_det;
...
model {
  ...
  target += reduce_sum(
    ...
    mean_lnA_det,                  // mean lnA per energy bin per source
    var_lnA_det,                   // variance of lnA per energy bin per source
...
generated quantities {
    ...
    for (i in 1:N) {
      ...
      real mean_lnA = mean_lnA_det[Ebin_idx];
      real var_lnA = var_lnA_det[Ebin_idx];
      real Zsrc = 0.5 * exp(mean_lnA + sqrt(var_lnA) * nu_lnAs[i]);
      real Rtrue = exp(logE_true[i]) / Zsrc;
      real kappa_gmf_interp = exp(interpolate(
        log10_gmf_Rgrid, log_kappa_gmf_grid[i], log10(Rtrue)
