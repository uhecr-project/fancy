...
class GMFBackPropagation:
    ...
    def __init__(self, data : Data, gmf_model="JF12"):
        ...
        if cr == None:
            raise ImportError("CRPropa is not installed and is required for using this module.")
        
        # uhecr direction properties
        self.uhecr_uv = (data.uhecr.coord.cartesian.xyz.value).T
        self.uhecr_coords_earth = SkyCoord(self.uhecr_uv, frame="galactic", representation_type="cartesian")
        self.uhecr_coords_earth.representation_type = "unitspherical"

        # uhecr rigidity properties
        self.uhecr_rigidity = data.uhecr.rigidity if len(data.uhecr.rigidity) > 0 else data.uhecr.energy / data.detector.meanZ
    ...
    def _get_time_delay(self, c, pos_earth):
        ...
        return ((c.getTrajectoryLength() -
                c.current.getPosition().getDistanceTo(pos_earth)) /
                cr.c_light / (60 * 60 * 24 * 365))
    ...
    def _setup_simulation(self, obs):
        ...
        sim.add(
            cr.PropagationCK(gmf_cr, 1e-4, 0.1 * cr.parsec,
                                100 * cr.parsec))
    ...
    def run_single_backpropagation(self, bt_arg):
        ...
        for k, uhecr_uv in enumerate(uhecr_uvs):
            ...
            c = cr.Candidate(
                cr.ParticleState(pid, uhecr_Rs[k], pos_earth,
                                    uhecr_vector3d))
            sim.run(c)
            ...
            uhecr_defl_uvs[k,:] = np.array([uhecr_defl_v3d.x, uhecr_defl_v3d.y, uhecr_defl_v3d.z])
        ...
        uhecr_mean_defl_uv = np.array([uhecr_mean_defl_v3d.x, uhecr_mean_defl_v3d.y, uhecr_mean_defl_v3d.z])
        uhecr_mean_defl_uv /= np.linalg.norm(uhecr_mean_defl_uv)  # normalise incase

        return uhecr_idx, uhecr_uvs, uhecr_defl_uvs, uhecr_mean_defl_uv, uhecr_time_delays
    ...
    def run_backpropagation(self, Nsamples=500, njobs=4, parallel=True):
        ...
        if parallel:
            results = Parallel(n_jobs=njobs)(delayed(self.run_single_backpropagation)(arg) for arg in bt_args)
        else:
            results = []
            for arg in bt_args:
        ...
        self.uhecr_coords_gb = SkyCoord(self.defl_mean_uvs, frame="galactic", representation_type="cartesian")
        self.uhecr_coords_gb.representation_type = "unitspherical"

    def compute_kappa_gmf(self):
        '''compute kappa gmf'''
        self.kappa_gmfs = Parallel(n_jobs=2)(delayed(self._get_kappa_gmf)(uhecr_idx) for uhecr_idx in range(self.Nuhecrs))
    ...
    def _get_kappa_gmf(self, uhecr_idx):
        ...
        fit = self.vMF_model.sample(
            data={"n" : self.defl_sampled_uvs[uhecr_idx,:,:], "N" : self.defl_sampled_uvs.shape[1], "mu":self.defl_mean_uvs[uhecr_idx,:]},
            iter_warmup=1000,
            iter_sampling=2000,
    ...
    def _f_theta_scalar(self, kappa, P=0.683):
        ...
        if kappa <= 1e5 and kappa > 1e-3:
            return np.arccos(1 + np.log((1 - P *
                                        (1 - np.exp(-2 * kappa)))) / kappa)
        elif kappa > 1e5:
