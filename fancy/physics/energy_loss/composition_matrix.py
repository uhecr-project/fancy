...
class CompositionMatrixContainer:
    ...
    massids = [101, 201, 302, 402, 904, 1005, 1105, 1206, 1306, 1407, 1507, 1608, 1708, 1808, 1909, 2010, 2110, 2210, 2311, 2412, 2512, 2612, 2713, 2814, 2914, 3014, 3115, 3216, 3316, 3416, 3517, 3618, 3718, 3818, 3919, 4019, 4119, 4220, 4320, 4420, 4521, 4622, 4722, 4822, 4922, 5023, 5123, 5224, 5324, 5424, 5525, 5626]

    def __init__(self, css="PSB", dmin = 0.8, dmax = 110, Nds = 100, resources_path : str = os.path.dirname(os.path.realpath(__file__)), nthreads=8):
        '''
        Container to handle all composition loss computation performed by Prince. In principle this only needs to be accessed if one wants to re-compute the composition weights.
        ...
        # get the kernel
        self.prince_run_datapath = os.path.join(self.resources_path, f"prince_run_{css}_mkl.ppo")
        ...
        self.redshift_distance_datapath = os.path.join(self.resources_path, "redshift_tables_Planck_WMAP.pkl")
        ...
        (_, self.spl_d_to_z_plk, _, _) = pickle.load(open(self.redshift_distance_datapath, "rb"))

        # initialise solver once to get utility functions to compute A and Z from massIDs
        solv = UHECRPropagationSolverBDF(initial_z=1.0, final_z = 0.0, prince_run=self.prince_run)
    ...
    def run_single_injection_solver(self, run_args):
        ...
        for massid in tqdm(massids):
            ...
            solver.add_source_class(NoInjection(prince_run, params={}))     # no source model
            solver.set_initial_state(massid, zinit)     # set uniform injection as initial state
            solver.solve(dz=min(zinit / 200, 3e-4), verbose=False, progressbar=False)   # solve
            # append for each mass
            results_per_dinit.append((solver.res, solver.initial_state[solver.spec_man.ncoid2sref[massid].sl]))
    ...
    def run_injection_solver(self, njobs=4, reset=False):
        ...
        if not os.path.exists(solver_res_path) or reset == True:
            ...
            run_args = [(i, self.distances[i], redshifts[i], self.prince_run, self.massids) for i in range(len(self.distances))]
    ...
    def compute_weights(self, solver_res):
        ...
        self.weights = np.zeros((len(self.distances), len(self.massids), len(self.massids), len(self.prince_run.cr_grid.grid)))
        ...
        self.weights_mg = np.zeros((len(self.distances), len(self.massids), 4, len(self.prince_run.cr_grid.grid)))

        self.determine_mass_groups()

        for img, (mg_lidx, mg_uidx) in enumerate(self.mass_group_idxlims):
            self.weights_mg[...,img,:] = np.sum(self.weights[...,mg_lidx:mg_uidx+1,:], axis=2) 
    ...
    def save(self, outfile):
        ...
        with h5py.File(outfile, "w") as f:
            ...
            weights_massgroup.create_dataset("mass_group_idxlims", data=np.array(self.mass_group_idxlims, dtype=int))
    ...
    def _create_kernel(self):
        ...
        cs = cross_sections.CompositeCrossSection([(0., cross_sections.TabulatedCrossSection, (self.css,)),
                                           (0.14, cross_sections.SophiaSuperposition, ())])
        
        # generate kernel
        prince_run = core.PriNCeRun(max_mass = 56, photon_field=self.pf_gilmore, cross_sections=cs)
    ...
    def _create_distance_tables(self):
        ...
        redshift_grid_plk = [
            z_at_value(Planck18.comoving_distance, d * u.Mpc)
            for d in distance_grid_mpc
        ]
        redshift_grid_wmap = [
        ...
        # Fix the zeros
        redshift_grid_plk.insert(0, 0.)
        redshift_grid_wmap.insert(0, 0.)
        distance_grid_mpc = np.hstack([[0], distance_grid_mpc])

        # computing both z-> d and d -> z
        spl_z_to_d_plk = UnivariateSpline(redshift_grid_plk, distance_grid_mpc, s=0, k=2)
        spl_d_to_z_plk = UnivariateSpline(distance_grid_mpc, redshift_grid_plk, s=0, k=2)
        spl_z_to_d_wmap = UnivariateSpline(redshift_grid_wmap, distance_grid_mpc, s=0, k=2)
        spl_d_to_z_wmap = UnivariateSpline(distance_grid_mpc, redshift_grid_wmap, s=0, k=2)
