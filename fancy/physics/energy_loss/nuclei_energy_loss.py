...
class NucleiEnergyLoss(EnergyLoss):
    ...
    def initialise_grid(
        ...
        self.weights = CubicSpline(x=rigidities.to_value(u.EV), y=weights_full, axis=-1)(self.rigidities_grid)
        self.weights_mg = CubicSpline(x=rigidities.to_value(u.EV), y=weights_full_mg[...,self.mass_group_idx,:], axis=-1)(self.rigidities_grid)

        if self.verbose:
            # initial size of wieghts
            print(f"Shape of weights from file: {weights_full.shape}")  # (Dsrc, Asrc, Aearths, R)
            print(f"Shape of mass-group integrated weights from file: {weights_full_mg.shape}")  # (Dsrc, Asrc, MG, R)

            # check if this makes sense
            print(f"Length of rigidity array for MG{self.mass_group}: {len(self.rigidities_grid)}")
            ...
            print(f"Shape of weights for MG{self.mass_group}: {self.weights.shape}")  # (Dsrc, Asrc, Aearths, R_mg)
            print(f"Shape of weights for MG{self.mass_group}: {self.weights_mg.shape}")  # (Dsrc, Asrc, R_mg)
    ...
    def _compute_source_PDF(self):
        ...
        self.Asrc_pdfs = np.zeros((self.Ndistances, self.NAsrcs, self.NRs))  # shape of (distances, source masses, rigidities)

        # iterate over each distance & rigidity
        for (id, ir) in np.ndindex((self.Ndistances, self.NRs)):
            self.Asrc_pdfs[id, :, ir] = self.weights_mg[id, :, ir] / np.sum(self.weights_mg[id, :, ir])
        
    def compute_source_spectrum(self):
        '''Computation of the source & arrival spectrum & mean energy'''
        self.src_spects_full = np.zeros((self.Ndistances, self.NAsrcs, self.NRs, self.Nalphas)) * (1 / u.EV)  # since its for all source compositions
        ...
        for (id, ia) in np.ndindex(self.Ndistances, self.Nalphas):
            ...
            for im in range(self.NAsrcs):
                ...
                src_spect_unnormed = self.Asrc_pdfs[id, im, :] * self.Zs[im]**(1 - self.alpha_grid[ia]) * self.rigidities_grid.to_value(u.EV)**(-self.alpha_grid[ia])

                # zeroths moment
                src_norm += np.trapz(y=src_spect_unnormed, x=self.rigidities_grid.to_value(u.EV))

            # normalisation is carried by contribution over all masses at the source
            self.src_spects_full[id, ..., ia] = (src_spect_unnormed / src_norm) * (1 / u.EV)
    ...
    def compute_arrival_spectrum(self):
        ...
        self.arr_spects_full = np.zeros((self.Ndistances, self.NAearths, self.NRs, self.Nalphas)) * (1 / u.EV) # here NAearths is for the *arrival composition*

        for (ime, ir, ia) in np.ndindex(self.NAearths, self.NRs, self.Nalphas):
            
            self.arr_spects_full[:, ime, ir, ia] = np.sum(self.weights[:, :, ime, ir] * self.src_spects_full[:, :, ir, ia], axis=1)

        self.arr_spects = np.sum(self.arr_spects_full[:,self.mg_lidx:self.mg_uidx+1,...], axis=1)
    ...
    def compute_Eexs(self):
        for (id, ia) in np.ndindex(self.Ndistances, self.Nalphas):
            ...
            for im in range(self.NAsrcs):
                ...
                src_norm += np.trapz(y=src_spect_unnormed, x=self.rigidities_grid.to_value(u.EV))
                src_Enorm += np.trapz(y=self.Zs[im] * self.rigidities_grid.to_value(u.EV) * src_spect_unnormed, x=self.rigidities_grid.to_value(u.EV))
    ...
    def p_gt_Rth(self, delta = None):
        ...
        return 1 - np.array([stats.norm.cdf(self.Rth, R, delta * R) for R in self.rigidities_grid.value])
    ...
    def save(self, outfile):
        ...
        with h5py.File(outfile, "a") as f:
            ...
            config_gr.create_dataset("log10_rigidities", data=np.log10(self.rigidities_grid.to_value(u.EV)))  # in log10(EV)
            config_gr.create_dataset("dRs_grid", data=self.dRs_grid.to_value(u.EV))

            config_gr.create_dataset("log10_arrspect_grid", data=np.log10(self.arr_spects.to_value(1 / u.EV)))
            ...
            config_gr.create_dataset("As_mg", data=self.As[self.mg_lidx:self.mg_uidx+1])
